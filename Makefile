...
all:
	...
	cd build/ && \
	docker build -t weather_api_rust/build_rust:ubuntu18.04 . && \
	cd ../ && \
	rm -rf build/

cleanup:
	docker rmi `docker images | python -c "import sys; print('\n'.join(l.split()[2] for l in sys.stdin if '<none>' in l))"`
	rm -rf /tmp/.tmp.docker.weather_api_rust
	rm Dockerfile

package:
	docker run --cidfile $(cidfile) -v `pwd`/target:/weather_api_rust/target weather_api_rust/build_rust:ubuntu18.04 \
        /weather_api_rust/scripts/build_deb_docker.sh $(version) $(release)
	docker cp `cat $(cidfile)`:/weather_api_rust/weather-api-rust_$(version)-$(release)_amd64.deb .
	docker rm `cat $(cidfile)`
	rm $(cidfile)

test:
	docker run --cidfile $(cidfile) -v `pwd`/target:/weather_api_rust/target weather_api_rust/build_rust:ubuntu18.04 /bin/bash -c ". ~/.cargo/env && cargo test"

build_test:
	cp Dockerfile.test.ubuntu18.04 build/Dockerfile && \
	cd build/ && \
	docker build -t weather_api_rust/test_rust:ubuntu18.04 . && \
	cd ../ && \
	rm -rf build/

install:
	cp target/$(build_type)/weather-api-rust /usr/bin/weather-api-rust
...
dev:
	docker run -it --rm -v `pwd`:/weather_api_rust rust_stable:latest /bin/bash || true
