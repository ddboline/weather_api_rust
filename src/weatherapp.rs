...
fn main() -> Result<(), Error> {
    ...
    loop {
        if WEATHER_CACHE.load().contains_key(DEFAULT_STR) {
            break;
        }
                        ...
                        div { class: "relative",
                            input { class: "p-2 pl-8 rounded border border-gray-200 bg-gray-200 focus:bg-white focus:outline-none focus:ring-2 focus:ring-yellow-600 focus:border-transparent",
                                ...
                                oninput: move |evt| {
                                    ...
                                    if let Some((weather, forecast)) = WEATHER_CACHE.load().get(msg) {
                                ...
                                onkeydown: move |evt| {
                                    if let Some((weather, forecast)) = WEATHER_CACHE.load().get(draft) {
                                        if let Some(weather) = weather {
                                            set_weather(weather.clone());
                                        }
                                        if let Some(forecast) = forecast {
